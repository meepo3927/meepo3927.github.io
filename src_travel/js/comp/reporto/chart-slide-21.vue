...
methods.fetchRender = function () {
	...
	this.fetchIndex(1, 'sceneryUserCreditAna').then(onOver).catch(onOver);
	this.fetchIndex(2, 'sceneryUserCreditAnaCompare').then(onOver).catch(onOver);
};
let computed = {};
computed.tip1_1 = function () {
	...
	return [
		'信用度' + copy[0].name + '的游客占比' + copy[0]._percent,
		'信用度' + copy[1].name + '的游客占比' + copy[1]._percent
	].join('，');
};
computed.tip1_2 = function () {
	...
	let str1 = '信用良好及以上游客占比最高的景区是' + m1.theName + '，占比' + m1.theStr;
	let str2 = '信用中等及以下游客占比最高的景区是' + m2.theName + '，占比' + m2.theStr;
