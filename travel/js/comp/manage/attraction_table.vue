<template>
<table class="table table-bordered " id="attraction_table">
    <thead>
        <tr>
            <th class="col-1"></th>
...
methods.requestChild = function (item) {
    let action = '/sceneryInfo/getSceneryDetail.action';
    let data = {sceneryId: item.sceneryId};
    ...
    item.loading = true;
    request.fetch2(url, {data}).then((data) => {
        item.children = data;
