...
methods.fetchRender = function () {
	...
	this.fetchIndex(1, 'sceneryUserGprsAvgDay').then(onOver).catch(onOver);
	this.fetchIndex(2, 'sceneryUserGprsDistribute').then(onOver).catch(onOver);
	this.fetchIndex(3, 'sceneryUserGprsCompare').then(onOver).catch(onOver);
};
let computed = {};
computed.tip1_1 = function () {
	...
	copy.forEach((v, i) => {
		if (v.name === '0M' && i > 0) {
			zeroText = '（0流量游客占' + v._percent + '）';
		}
	});
	return [
		'景区内日户均' + copy[0].name + '的流量游客占比最多，达' + copy[0]._percent,
		zeroText
	].join('');
};
...
computed.tip2 = function () {
	...
	let first = copy[0].name + '上网流量需求最大，达到'
