...
computed.t1 = function () {
	if (this.isDimScenery) {
		return '本分析报告主要针对景区的游客到访情况进行分析。';
	} else {
		let cityName = this.mCondition.city_name || '城';
		return '本分析报告主要针对整个' + cityName + '市的游客情况进行分析。';
	}
};
...
computed.dateStr = function () {
	if (this.isDimScenery) {
		...
		if (this.isYearDateType) {
			return [
				this.mCondition.year + '年01月',
				this.mCondition.year + '年12月'
			].join('—')
		} else if (this.isMonthDateType) {
			return this.mCondition.year + '年'
				+ this.mCondition.month + '月';
		} else {
			let start = buildDateStr(this.mCondition.start_date);
			let end = buildDateStr(this.mCondition.end_date);
			return start + '—' + end;
